...
import { Share2, Trophy, TrendingUp, Copy, Check, LogOut, Crown, User as UserIcon } from 'lucide-react';
import { db } from '../firebase';
import { ref, push, set, onValue, get } from 'firebase/database';
import { ACTIVATION_FEE, SALARY_TARGET, SALARY_AMOUNT } from '../constants';
...
export const HomeView: React.FC<HomeViewProps> = ({ user, onLogout, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    // Calculate referrals for salary progress
    const usersRef = ref(db, 'users');
    get(usersRef).then(snapshot => {
        if (snapshot.exists()) {
            const users = Object.values(snapshot.val()) as User[];
            // Case insensitive comparison for referral code
  ...
  // Internal logic uses SALARY_TARGET (3000), but Marketing shows 2000
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-3xl p-6 text-white shadow-lg relative overflow-hidden">
          ...
          <div className="relative z-10">
              ...
              <p className="text-[10px] text-emerald-100 mt-2 text-center">
                  Reach {MARKETING_TARGET} active referrals to unlock ৳{SALARY_AMOUNT} monthly salary!
              </p>
          </div>
      </div>
